...
var searchSequence$ = rxjs_1.fromEvent(input, 'keyup');
var request$ = function (searchValue) {
    return rxjs_1.from(fetch("https://api.github.com/search/repositories?q=" + searchValue)
        .then(function (res) { return res.json(); }));
};
var renderResult = function (response) {
    var resultContainer = document.getElementById('search_results');
    var resultContent = document.createElement('div');
    var createRepoItem = function (repoItem) {
        var item = document.createElement('div');
        item.innerHTML = "<a href=\"" + repoItem.html_url + "\" target=\"_blank\">" + repoItem.name + "</a>";
        return item;
    };
    response.items.forEach(function (item) {
        var repoItem = createRepoItem(item);
...
searchSequence$.pipe(operators_1.map(function (ev) { return ev.target.value; }), operators_1.filter(function (value) { return !!value; }), operators_1.tap(function (val) { return console.log(val); }), operators_1.debounceTime(1000), operators_1.tap(function (val) { return console.log('afterDebounce: ', val); }), operators_1.switchMap(function (value) { return request$(value); })).subscribe(function (response) {
